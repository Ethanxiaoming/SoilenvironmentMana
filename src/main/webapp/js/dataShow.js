	...
	$('#tableshow').submit(function() {
		...
		if(detailKind=="region"){
			$.ajax({
				...
				type : "POST",
				data: "year="+ year +"&region="+ region+ "&towns="+ towns ,
				dataType : 'json',
				async : false,
				success : function(data) {
			...
			$.ajax({
				...
				type : "POST",
				data: "year="+ year +"&regionName="+ region+ "&towns="+ towns ,
				dataType : 'json',
				async : false,
				success : function(data) {
			    ...
			    xAxis: [
			        {
			            type: 'category',
			            axisTick: {show: false},
			            data:  [ '东丽', '津南', '北辰', '西青', '汉沽', '塘沽', '大港', '宝坻', '宁河','蓟县', '静海', '武清', '天津市' ]
			        }
			    ],
			    yAxis: [
			        {
