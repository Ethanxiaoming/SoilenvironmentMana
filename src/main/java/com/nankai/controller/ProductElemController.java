...
public class ProductElemController {
	...
	public ResultInfoVO eleConOfsections(String year, String regionName) {
		...
		// List<String>
		// productsmap=productelementservice.findproductOfRegion(thisyear,
		// regionName);
	...
	@ResponseBody
	public ResultInfoVO infoOfProduct(String year, String regionName, String towns) {
		ResultInfoVO vo = new ResultInfoVO();
		int thisyear = Integer.parseInt(year);
		List<BasicInfo> result = new ArrayList<>();
		if (towns.equals("全部"))
			result = productelementservice.findBasicInfo(thisyear, regionName);
		else
			result = productelementservice.findBasicInfo(thisyear, regionName, towns);
		Map<String, Object> map = new HashMap();
