...
public class SecionElemController {
	...
	public ResultInfoVO eleConofregionforyears(String kind, String regionName) {
		...
		if(regionName.equals("天津市区")||regionName.equals("天津市")){
	...
	@ResponseBody
	public ResultInfoVO selectAllofRegion(String year, String region,String towns) {
		int thisyear = Integer.parseInt(year);
		ResultInfoVO vo = new ResultInfoVO();
		List<BasicInfo> allresult = new ArrayList<>();
		if(towns.equals("全部"))
			allresult = sectionelementservice.findBasicInfo(thisyear, region);
		else
			allresult = sectionelementservice.findBasicInfo(thisyear, region, towns);
		Map<String, Object> map = new HashMap();
