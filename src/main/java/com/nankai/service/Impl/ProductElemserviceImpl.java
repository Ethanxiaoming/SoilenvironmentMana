...
public class ProductElemserviceImpl implements ProductElemService {
	...
	public Map<String, Object> findEleStatisOfRegion(int year, String regionName, int eleid) {
		...
		if (maxmap == null || avgmap == null) {
			...
			resultmap.put("maxcontent", minmap.get("mincontent").toString()=="ND"?"-":df.format(Double.parseDouble(maxmap.get("maxcontent").toString())));
	...
	public List<BasicInfo> findBasicInfo(int year, String regionName) {
		...
		for (int i = 0; i < result.size(); i++) {
			...
			result.get(i).setElemCd(
					result.get(i).getElemCd().equals("0") ? "0" : df.format(Double.valueOf(result.get(i).getElemCd())));
			result.get(i).setElemHg(
					result.get(i).getElemHg().equals("0") ? "0" : df.format(Double.valueOf(result.get(i).getElemHg())));
			result.get(i).setElemAs(
					result.get(i).getElemAs().equals("0") ? "0" : df.format(Double.valueOf(result.get(i).getElemAs())));
			result.get(i).setElemCu(
					result.get(i).getElemCu().equals("0") ? "0" : df.format(Double.valueOf(result.get(i).getElemCu())));
			result.get(i).setElemPb(
					result.get(i).getElemPb().equals("0") ? "0" : df.format(Double.valueOf(result.get(i).getElemPb())));
			result.get(i).setElemCr(
					result.get(i).getElemCr().equals("0") ? "0" : df.format(Double.valueOf(result.get(i).getElemCr())));
			result.get(i).setElemZn(
					result.get(i).getElemZn().equals("0") ? "0" : df.format(Double.valueOf(result.get(i).getElemZn())));
			result.get(i).setElemNi(
					result.get(i).getElemNi().equals("0") ? "0" : df.format(Double.valueOf(result.get(i).getElemNi())));
		}
		return result;
	}

	@Override
	public List<BasicInfo> findBasicInfo(int year, String regionName, String towns) {
		List<BasicInfo> result = productdao.selectProductInfobytowns(year, regionName, towns);
		DecimalFormat df = new DecimalFormat("0.00000");// 设置double后小数点的位数
		for (int i = 0; i < result.size(); i++) {
			result.get(i).setName(regionName);
			result.get(i).setYear(year);
